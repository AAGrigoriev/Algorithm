...
private:
  ...
  bst_node* search_impl(bst_node* root, const Key& val);
};


template<typename Key>
typename BST<Key>::bst_node* BST<Key>::find_min_node(bst_node* const root) {
  auto min = root;
  while (min && min->left) {
    min = min->left;
  }
...
template <typename Key>
typename BST<Key>::bst_node* BST<Key>::search(const Key& val) {
  return search_impl(root_, val);
}


template <typename Key>
typename BST<Key>::bst_node* BST<Key>::search_impl(bst_node* root, const Key& val) {
